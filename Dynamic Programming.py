...
# given an array of n numbers, find M(i) that indicates max sum over all windows starting at i and ending at n.
# time O(n) space O(n)

# given an array of n numbers, find M(i) that indicates max sum over all windows starting at 1 and ending at i.
# condition: not to select two contiguous numbers. time O(n) space O(n)

# given an array of n numbers, find M(i) that indicates max sum over all windows starting at i and ending at n.
# condition: not to select two contiguous numbers. time O(n) space O(n)

# given an array of n numbers, find M(i) that indicates max sum over all windows starting at 1 and ending at i.
# condition: not to select three contiguous numbers. time O(n) space O(n)
...
# around circuit once , otherwise return -1.
...
# Box Stacking: given a set of n rectangular 3D boxes. Dimensions of ith box are height hi, widhth wi, depth di.
